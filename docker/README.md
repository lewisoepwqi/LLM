...
> **业务后端接入请看 [`API.md`](API.md)** —— 完整接口文档：请求/响应格式、结构化输出、
> 上下文预算、性能与并发实测、prompt cache 用法、错误处理、Python/SDK 代码示例。
> 下面几条是最容易踩的坑，摘要版。
...
| 两路并发峰值 | **9016 MiB，增量仅 2 MiB** |
| `prompt_tokens` | 3044（单路）/ 3041（并发）——贴着 3072 上限，`--image-max-tokens` 确实吃满 |
| 单路端到端 | 7.5 秒（prefill 606 tok/s，decode 53.7 tok/s） |
| 两路并发 | 11.5 秒（串行应为 15 秒，吞吐约 +30%） |
| 纯文本回归 | 正常，无 `<think>` 段，54.9 tok/s |
| 字段准确率 | `testdata/contract_scan.png` **6/6** |
| 回滚演练 | 通过（注释 6 行 → 视觉参数归 0 → 恢复逐字节一致） |

**三条值得记住的结论：**

1. **显存不会因并发膨胀。** 两路同时做满载视觉编码，峰值只比单路高 2 MiB。原因是
   mmproj 的 worst-case 缓冲（启动日志报 1148.64 MiB）在**加载时一次性预分配**，
   并发编码复用同一块，不是各要一份。所以没有并发导致 OOM 的风险——这比"还剩 7 GB
   余量"更可靠，余量可能被同卡的其它服务吃掉，"并发不涨"是结构性的。
2. **并发提升吞吐约 30%，不是翻倍。** 两个槽共享同一块 GPU 算力，`--parallel` 省的是
   调度和 KV 管理开销，不是算力。视觉 prefill 是算力密集型。后端 `READ_CONCURRENCY=2`
   合理，但别指望线性扩展；要更高并发得加卡，瓶颈在算力不在显存。
3. **prompt cache 对多轮追问收益很大。** 同一张图第二次提问时实测
   `cached_tokens 3011 / prompt_tokens 3015`，耗时从 7.5 秒降到 1.4 秒——视觉编码只做
   一次。合同抽取常见的"先问甲乙方、再问付款条款、再问违约责任"这种追问模式能直接受益。
   跨文档命中率低（每份扫描件不同），文档内多轮是实打实的。
   *测性能时注意用不同的提示词打散缓存，否则测到的是缓存命中而不是真实编码。*

**与上线前估算的偏差**（供后续调参参考）：

- 计划里估的峰值 11–12 GiB **明显偏保守**，实测本服务约 8.4 GiB（9016 减去同卡他人的 569）。
- 计划里定的「不设 `--image-min-tokens`，保持默认」**被运行时警告推翻**：llama.cpp 加载
  Qwen-VL 时明确建议设为 1024，实测 224×224 小图只占约 48 个视觉 token，远低于该下限。已加。

**尚未验证**：`testdata/README.md` 里更难的那批字段——18 位统一社会信用代码、旋转
半透明印章上的文字、第 3 行规格型号——本次未实测。公司名和日期这类字段模型靠语言先验
就能补全，6/6 的说服力有限。要评估真实 OCR 上限，建议补跑一次只问长数字串的抽取。
